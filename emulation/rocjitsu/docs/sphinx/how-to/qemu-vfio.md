...
myst:
    html_meta:
        "description": "Run and qualify a gfx1250 Linux guest with QEMU and rocJITsu's VFIO-user PCI device."
        "keywords": "rocJITsu, QEMU, VFIO-user, gfx1250, AMDGPU, ROCm, GEMM"
---

# Run a QEMU VFIO-user compute guest
...
server after QEMU exits and fails if the server does not stop cleanly. A
nonzero server exit fails the run immediately. A clean exit means the guest
disconnected, so the launcher lets QEMU finish and evaluates its status and
guest log normally.
...
failure immediately; startup speed does not require a timeout policy.
