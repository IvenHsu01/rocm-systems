...
def require_qemu_vfio_user(qemu: Path) -> None:
    """Require the QEMU device model used by this launcher."""

    result = subprocess.run(
        [str(qemu), "-device", "help"],
        text=True,
        capture_output=True,
        check=False,
        timeout=CAPABILITY_PROBE_TIMEOUT,
...
def require_rocjitsu_vfio_user(rocjitsu: Path) -> None:
    """Require a rocjitsu binary built with its VFIO-user front end."""

    result = subprocess.run(
        [str(rocjitsu), "--check-vfio-user"],
        text=True,
        capture_output=True,
        check=False,
        timeout=CAPABILITY_PROBE_TIMEOUT,
...
def start_vfio_server(
    arguments: list[str], server_log: Any
...
def wait_for_server_ready(
    path: Path, process: subprocess.Popen[Any], ready_fd: int
) -> SocketIdentity:
...
def main(arguments: list[str] | None = None) -> int:
    ...
    try:
        ...
        require_qemu_vfio_user(qemu)
        require_rocjitsu_vfio_user(rocjitsu)
        ...
        with server_log_path.open("wb") as server_log:
            server, ready_fd = start_vfio_server(server_arguments, server_log)
        run_error: GuestRunError | OSError | subprocess.SubprocessError | None = None
        socket_identity: SocketIdentity | None = None
        try:
            try:
                socket_identity = wait_for_server_ready(socket_path, server, ready_fd)
            finally:
                os.close(ready_fd)
