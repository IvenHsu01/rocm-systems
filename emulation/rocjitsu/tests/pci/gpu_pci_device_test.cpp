...
TEST(GpuDeviceTopology, AcceptsTheZeroEncodingForOneEngineAndOneBackend) {
  rocjitsu::GpuPciDeviceSpec spec = configured_spec();
  spec.discovery.graphics.shader_engines = 1;
  spec.discovery.graphics.shader_arrays_per_engine = 1;
  spec.discovery.graphics.render_backends_per_engine = 1;
  spec.discovery.graphics.shader_complexes_per_engine = 1;

  rocjitsu::GpuPciDevice device("one-by-one", spec, nullptr);
  ASSERT_TRUE(device.usable());

  constexpr uint64_t kGbAddrConfigRead = (0x1260 + 0x13e2) * sizeof(uint32_t);
  std::array<std::byte, sizeof(uint32_t)> raw{};
  ASSERT_EQ(device.bar_access(rocjitsu::GpuPciDevice::kRegisterBar, raw, kGbAddrConfigRead,
                              /*write=*/false),
            static_cast<int64_t>(raw.size()));
  EXPECT_EQ(std::bit_cast<uint32_t>(raw), 0u);
}
...
public:
  [[nodiscard]] bool read(uint64_t guest_phys, std::span<std::byte> dst) override {
    ...
    for (std::size_t i = 0; i < dst.size(); ++i) {
      const auto found = memory_.find(guest_phys + i);
      dst[i] = found == memory_.end() ? std::byte{0} : found->second;
    }
    return true;
  }
  ...
  simdojo::DmaAccessOutcome read_outcome(uint64_t guest_phys, std::span<std::byte> dst) override {
    {
      const std::lock_guard lock(memory_mutex_);
      if (next_read_outcome_ && next_read_outcome_->first == guest_phys) {
        const simdojo::DmaAccessOutcome outcome = next_read_outcome_->second;
  ...
  [[nodiscard]] bool write(uint64_t guest_phys, std::span<const std::byte> src) override {
    const std::lock_guard lock(memory_mutex_);
    if (refuse_writes_ || writes_before_refusing_ == 0) {
      return false;
    }
    if (writes_before_refusing_ > 0) {
      --writes_before_refusing_;
    }
    writes_.emplace_back(guest_phys, src.size());
    for (std::size_t i = 0; i < src.size(); ++i) {
      memory_[guest_phys + i] = src[i];
    }
    return true;
  }
  ...
  simdojo::DmaAccessOutcome write_outcome(uint64_t guest_phys,
    ...
    {
      const std::lock_guard lock(memory_mutex_);
      if (next_write_outcome_ && next_write_outcome_->first == guest_phys) {
        const simdojo::DmaAccessOutcome outcome = next_write_outcome_->second;
        next_write_outcome_.reset();
        return outcome;
      }
    }
  ...
  [[nodiscard]] bool trigger(uint32_t vector) override {
    const std::lock_guard lock(memory_mutex_);
    triggered_.push_back(vector);
    return !refuse_trigger_;
  }
  ...
  template <typename T> void store_value(uint64_t guest_phys, T value) {
    const auto raw = std::bit_cast<std::array<std::byte, sizeof(T)>>(value);
    const std::lock_guard lock(memory_mutex_);
    for (std::size_t i = 0; i < raw.size(); ++i)
      memory_[guest_phys + i] = raw[i];
  }
  ...
  [[nodiscard]] bool memory_empty() const {
    const std::lock_guard lock(memory_mutex_);
  ...
  [[nodiscard]] std::vector<std::pair<uint64_t, std::size_t>> writes() const {
    const std::lock_guard lock(memory_mutex_);
    return writes_;
  }

  void fail_next_read(uint64_t guest_phys, simdojo::DmaAccessOutcome outcome) {
    const std::lock_guard lock(memory_mutex_);
    next_read_outcome_ = std::pair{guest_phys, outcome};
  }

  void fail_next_write(uint64_t guest_phys, simdojo::DmaAccessOutcome outcome) {
    const std::lock_guard lock(memory_mutex_);
    next_write_outcome_ = std::pair{guest_phys, outcome};
  }

  void refuse_writes(bool refuse) {
    const std::lock_guard lock(memory_mutex_);
    refuse_writes_ = refuse;
  }

  void accept_writes_before_refusing(int count) {
    const std::lock_guard lock(memory_mutex_);
    writes_before_refusing_ = count;
  }

  void refuse_trigger(bool refuse) {
    const std::lock_guard lock(memory_mutex_);
    refuse_trigger_ = refuse;
  }
  ...
  [[nodiscard]] std::vector<uint32_t> triggered() const {
    const std::lock_guard lock(memory_mutex_);
    return triggered_;
  }
  ...
  [[nodiscard]] bool wait_for_blocked_read(std::chrono::milliseconds timeout) {
    std::unique_lock lock(read_mutex_);
    return read_state_changed_.wait_for(lock, timeout, [this]() { return read_is_blocked_; });
  }
...
private:
  ...
  std::map<uint64_t, std::byte> memory_;
  ...
  std::vector<std::pair<uint64_t, std::size_t>> writes_;
  std::vector<uint32_t> triggered_;
  bool refuse_writes_ = false;
  bool refuse_trigger_ = false;
  /// @brief Writes to accept before refusing, or negative for no limit.
  int writes_before_refusing_ = -1;
  std::optional<std::pair<uint64_t, simdojo::DmaAccessOutcome>> next_read_outcome_;
  std::optional<std::pair<uint64_t, simdojo::DmaAccessOutcome>> next_write_outcome_;
...
TEST(VramStore, RejectsInvalidApertureSizesBeforeMapping) {
  const rocjitsu::VramStore zero_aperture("zero-aperture", 0x2000, 0);
  EXPECT_FALSE(zero_aperture.usable());
  EXPECT_TRUE(zero_aperture.aperture().empty());

  const rocjitsu::VramStore oversized_aperture("oversized-aperture", 0x1000, 0x2000);
  EXPECT_FALSE(oversized_aperture.usable());
  EXPECT_TRUE(oversized_aperture.aperture().empty());

  const rocjitsu::VramStore zero_backing("zero-backing", 0, 0);
  EXPECT_FALSE(zero_backing.usable());
}
...
protected:
  ...
  void configure_gart() {
    ...
    for (const auto [page, physical] :
      ...
      const uint64_t pte = physical | 0x63;
  ...
  void store_dword(uint64_t physical, uint32_t value) { transport_.store_value(physical, value); }

  void store_qword(uint64_t physical, uint64_t value) { transport_.store_value(physical, value); }
...
TEST_F(GpuDeviceMes, ResetDropsDoorbellsQueuedInTheSameTransportGeneration) {
  ...
  device_.reset(simdojo::ResetKind::FunctionLevel);
  ASSERT_NE(device_.transport_session_generation(), generation);
  device_.drain_doorbell_inbox_for_test();
...
TEST_F(GpuDeviceMes, RetainsADoorbellQueuedWhileAnEarlierNotificationIsDraining) {
  ...
  const bool blocked = transport_.wait_for_blocked_read(std::chrono::seconds(1));
  if (!blocked)
...
TEST_F(GpuDeviceMes, RetriesRemoveQueueCompletionWithoutReplayingRemoval) {
  ...
  transport_.fail_next_write(kRingFencePhysical, simdojo::DmaAccessOutcome::Unavailable);
...
TEST_F(GpuDeviceMes, RejectsUnsupportedGraphicsQueueWithoutChangingCoreState) {
  store_dword(kRingPhysical, 0x00040021);
  store_dword(kRingPhysical + 28 * sizeof(uint32_t), 0);
  store_qword(kRingPhysical + 38 * sizeof(uint32_t), kApiStatusGpu);
  store_qword(kRingPhysical + 40 * sizeof(uint32_t), 1);
...
TEST_F(GpuDeviceMes, RetriesUnavailableAddQueueMqdBeforeCommittingTheQueue) {
  ...
  transport_.fail_next_read(kSchedulerMqdPhysical + kMqdFirstDword * sizeof(uint32_t),
...
TEST_F(GpuDeviceMes, RetriesRemoveQueueReadPointerWithoutReplayingRemovalOrCompletion) {
  ...
  transport_.clear_writes();
  transport_.fail_next_write(kReadPointerPhysical, simdojo::DmaAccessOutcome::Unavailable);
  mes_doorbell = std::bit_cast<std::array<std::byte, 8>>(uint64_t{128});
  ring_doorbell(0x60, mes_doorbell);
...
TEST_F(GpuDeviceMes, TerminalRemoveQueueCompletionFailureDoesNotRetryOrReplayRemoval) {
  ...
  transport_.clear_writes();
  transport_.fail_next_write(kRingFencePhysical, simdojo::DmaAccessOutcome::Faulted);
  mes_doorbell = std::bit_cast<std::array<std::byte, 8>>(uint64_t{128});
  ring_doorbell(0x60, mes_doorbell);
...
TEST_F(GpuDeviceMes, RetriesComputeRingFetchWithoutReplayingCompletedPackets) {
  ...
  transport_.fail_next_read(kComputeRingPhysical + 3 * sizeof(uint32_t),
                            simdojo::DmaAccessOutcome::Unavailable);
...
TEST_F(GpuDeviceMes, RetriesComputeReadPointerWithoutReplayingRegisterWrites) {
  ...
  transport_.fail_next_write(kComputeSecondPagePhysical + 0xff0,
                             simdojo::DmaAccessOutcome::Unavailable);
  auto compute_doorbell = std::bit_cast<std::array<std::byte, 4>>(uint32_t{256});
  ring_doorbell(kComputeDoorbell, compute_doorbell);
...
TEST_F(GpuDeviceMes, DefersComputeQueueRemovalUntilCommittedProgressIsPublished) {
  ...
  store_dword(kComputeRingPhysical + 4, 0x40);
  store_dword(kComputeRingPhysical + 8, 0xdeadbeef);
  transport_.fail_next_write(kComputeReadPointerPhysical, simdojo::DmaAccessOutcome::Unavailable);
...
TEST_F(GpuDeviceMes, PermanentPm4CursorFaultMakesRemoveQueueTerminalUntilReset) {
  ...
  transport_.fail_next_write(kDefaultComputeReadPointerPhysical,
                             simdojo::DmaAccessOutcome::Faulted);
...
TEST(GpuDeviceLifetime, ReplacementFrontendReusesPasidAndDoorbellAfterDestruction) {
  ...
  const auto store_dword = [&transport](uint64_t address, uint32_t value) {
    transport.store_value(address, value);
  };
  ...
  const auto configure_and_add_queue = [&](rocjitsu::GpuPciDevice &device) {
    ...
    for (const auto [page, physical] :
      ...
      if (!write_vram_qword(device, kPageTable + page * sizeof(uint64_t), physical | 0x63))
        return std::optional<rocjitsu::amdgpu::AddressSpaceHandle>{};
    }
...
TEST_F(GpuDeviceSdma, ExecutesTheFirmwareFreeSdmaRingAfterA32BitDoorbell) {
  constexpr uint64_t kSdmaBase = 0x1260;
  ...
  auto doorbell = std::bit_cast<std::array<std::byte, 4>>(uint32_t{64});
...
TEST_F(GpuDeviceSdma, ResetWaitsForAdmittedPciEffectBeforeResettingRegisters) {
  ...
  struct ScopedGateRelease {
    rocjitsu::GpuPciDevice &device;
    std::jthread &drain;
    std::mutex &mutex;
    ...
    ~ScopedGateRelease() {
      ...
      if (drain.joinable())
        drain.join();
      device.set_sdma_pci_effect_admitted_hook_for_test({});
    }
  } cleanup{device_, drain, gate_mutex, gate_changed, release};
  {
    std::unique_lock lock(gate_mutex);
...
TEST_F(GpuDeviceSdma, RetriesUnavailableRingFetchWithoutAnotherDoorbell) {
  ...
  store_dword(kSchedulerRingPhysical + 12, 0);
  store_dword(kSchedulerRingPhysical + 16, 0xdeadbeef);
  transport_.fail_next_read(kSchedulerRingPhysical, simdojo::DmaAccessOutcome::Unavailable);
...
TEST_F(GpuDeviceSdma, RegisterBackedQueueRejectsAddressesOutsideTheGartAperture) {
  constexpr uint64_t kSdmaBase = 0x1260;
  ...
  // VMID 0 is bounded by the programmed GART aperture. An address outside it
  // must fault rather than being reinterpreted as a local-memory address.
  ...
  EXPECT_EQ(vram_dword_at(kLocalDestination), 0u);
  EXPECT_EQ(vram_dword_at(kLocalReadPointer), 0u);
...
TEST_F(GpuDeviceSdma, TerminalFaultSurvivesUnavailableReadPointerPublication) {
  ...
  transport_.fail_next_read(kSchedulerApiStatusPhysical, simdojo::DmaAccessOutcome::Faulted);
  transport_.fail_next_write(kSchedulerReadPointerPhysical, simdojo::DmaAccessOutcome::Unavailable);
...
TEST_F(GpuDeviceDelivery, MarshalsCommandProcessorInterruptsOntoThePciOwnerThread) {
  const std::thread::id owner_thread = std::this_thread::get_id();
  std::thread::id producer_thread;
  std::jthread producer([&] {
    producer_thread = std::this_thread::get_id();
    device_.deliver_cp_interrupt_for_test(7, 0xfeedface);
  });
  producer.join();

  EXPECT_TRUE(transport_.triggered().empty())
      << "the command processor delivered through PCI from its producer thread";
  device_.drain_doorbell_inbox_for_test();

  EXPECT_EQ(device_.last_cp_interrupt_thread_for_test(), owner_thread);
  EXPECT_NE(device_.last_cp_interrupt_thread_for_test(), producer_thread);
  ASSERT_EQ(transport_.triggered().size(), 1u);
  EXPECT_EQ(transport_.dword_at(kRingBase + 3 * sizeof(uint32_t)) & 0xffffu, 7u);
  EXPECT_EQ(transport_.dword_at(kRingBase + 4 * sizeof(uint32_t)), 0xfeedfaceu);
}
...
TEST_F(GpuDeviceDelivery, NeverWritesPastTheRingHoweverTheGuestSetsThePointer) {
  for (const uint32_t hostile : {static_cast<uint32_t>(kRingBytes - 1),
    ...
    transport_.clear_memory();
    transport_.clear_writes();
    transport_.clear_triggered();
...
TEST_F(GpuDeviceDelivery, RaisesNothingWhenThePointerCannotBePublished) {
  transport_.accept_writes_before_refusing(1);
...
TEST_F(GpuDeviceDelivery, DeclinesARingItCannotSafelyWriteTo) {
  ...
  EXPECT_TRUE(transport_.triggered().empty());
  EXPECT_TRUE(transport_.memory_empty()) << "nothing was written anywhere";
}
...
TEST_F(GpuDeviceDelivery, DeclinesWithNoTransportAttached) {
  ...
  EXPECT_TRUE(transport_.memory_empty());
}
...
TEST_F(GpuDeviceDelivery, RefusesASecondTransportAndKeepsServingTheFirst) {
  ...
  EXPECT_TRUE(interloper.memory_empty()) << "the refused transport was still reached";
  EXPECT_TRUE(interloper.triggered().empty()) << "the refused transport was still raised";
  EXPECT_FALSE(transport_.memory_empty()) << "the owning transport stopped being served";
...
TEST(GpuDeviceFlushes, RefusesARequiredPublishedBlockWithoutRegisterBases) {
  rocjitsu::GpuPciDeviceSpec spec = configured_spec();
  bool removed_bases = false;
  for (rocjitsu::IpBlock &block : spec.discovery.blocks) {
    if (block.hardware_id == rocjitsu::IpHardwareId::MmHub) {
      block.register_bases.clear();
      removed_bases = true;
    }
  }
  ASSERT_TRUE(removed_bases) << "the profile has no MMHUB record to exercise";

  const rocjitsu::GpuPciDevice incomplete("incomplete", spec, nullptr);
  EXPECT_FALSE(incomplete.usable())
      << "the device advertised a required block whose registers cannot be addressed";
}
...
TEST(GpuDeviceInterruptRing, RefusesAnOsssysVersionWithNoKnownLayout) {
  ...
  ASSERT_TRUE(changed_version) << "the profile has no OSSSYS record to exercise";
...
TEST_F(GpuDeviceDelivery, ReportsAnOverflowRatherThanSilentlyOverwriting) {
  ...
  ASSERT_TRUE(
      device_.write_register_for_test(0x4480, kIhRingControl | kIhWritePointerOverflowClear))
      << "the packet-side register path rejected the IH overflow-clear write";
