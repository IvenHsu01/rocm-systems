...
TEST(Pm4PacketProcessorTest, RejectsCountDerivedTwoDwordNop) {
  Pm4PacketProcessor processor;
  const std::array<uint32_t, 2> packet = {0xc0001000, 0};

  const Pm4PacketProcessResult result = processor.process({.available_dwords = packet});

  EXPECT_EQ(result.packet.status, PacketProcessStatus::Unsupported);
  EXPECT_EQ(result.packet.retirement, PacketRetirement::Hold);
  EXPECT_EQ(result.packet.retirement_bytes, 0u);
...
TEST(Pm4PacketProcessorTest, RejectsUndefinedRegisterOffsetBits) {
  uint32_t writes = 0;
  Pm4PacketProcessor processor({.write_uconfig_register = [&](uint64_t, uint32_t) {
    ++writes;
    return Pm4RegisterWriteStatus::Complete;
  }});
  const std::array<uint32_t, 3> packet = {0xc0017900, 0x00010040, 0xdeadbeef};

  const Pm4PacketProcessResult result = processor.process({.available_dwords = packet});

  EXPECT_EQ(result.packet.status, PacketProcessStatus::Malformed);
  EXPECT_EQ(result.packet.retirement, PacketRetirement::Hold);
  EXPECT_EQ(result.diagnostic, Pm4PacketDiagnostic::InvalidRegisterOffset);
  EXPECT_EQ(writes, 0u);
}
