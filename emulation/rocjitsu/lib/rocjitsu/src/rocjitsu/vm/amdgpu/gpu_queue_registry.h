...
public:
  ...
  /// @details Return Ready when destruction is safe, Busy when retry-critical
  /// state still needs publication, or Faulted when graceful removal cannot
  /// complete. Busy and Faulted keep the handle live for a retry or forced close.
...
private:
  ...
  public:
    RegistrationTransaction(GpuQueueRegistry &registry, uint32_t process_id, uint32_t queue_id)
        : registry_(registry), identity_(process_id, queue_id) {}
  ...
  private:
    ...
    std::pair<uint32_t, uint32_t> identity_;
  };
  ...
  [[nodiscard]] bool identity_exists_locked(const QueueIdentity &identity) const;
  [[nodiscard]] bool identity_is_pending_locked(const QueueIdentity &identity) const;
  ...
  void finish_registration(std::pair<uint32_t, uint32_t> identity);
  ...
  std::set<std::pair<uint32_t, uint32_t>> pending_identities_;
