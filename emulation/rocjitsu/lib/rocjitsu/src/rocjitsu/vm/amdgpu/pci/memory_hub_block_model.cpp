...
void MemoryHubBlockModel::observe_register_write(uint64_t byte_offset, PciMemoryAccess &memory) {
  for (uint32_t engine = 0; engine < kInvalidationEngines; ++engine) {
    ...
    // VMID 0's GART registers belong to GFXHUB. MMHUB still participates in
    // the driver's flush protocol, but has no independent translator to
    // publish from its zero-valued configuration registers.
    if ((requested & 1u) != 0 && layout_->id == IpHardwareId::Gc) {
      completed &= ~1u;
      if (soc_ != nullptr) {
