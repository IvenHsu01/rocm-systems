...
GpuQueueRegistry::RegistrationTransaction::~RegistrationTransaction() {
  registry_.finish_registration(identity_);
}
...
bool GpuQueueRegistry::identity_exists_locked(const QueueIdentity &identity) const {
  return std::ranges::any_of(slots_, [&identity](const Slot &slot) {
    return slot.queue != nullptr && slot.queue->state != QueueState::Dead &&
           slot.queue->identity.process_id == identity.process_id &&
           slot.queue->identity.queue_id == identity.queue_id;
  });
}

bool GpuQueueRegistry::identity_is_pending_locked(const QueueIdentity &identity) const {
  return pending_identities_.contains({identity.process_id, identity.queue_id});
}
...
void GpuQueueRegistry::finish_registration(std::pair<uint32_t, uint32_t> identity) {
  std::lock_guard lock(mutex_);
  assert(active_registrations_ != 0);
  const std::size_t erased = pending_identities_.erase(identity);
  assert(erased == 1);
  (void)erased;
...
QueueHandle GpuQueueRegistry::register_queue(const QueueRegistrationRequest &request) {
  ...
  {
    std::lock_guard lock(mutex_);
    if (!admission_open_ || identity_exists_locked(request.identity) ||
        identity_is_pending_locked(request.identity))
      return {};
    pending_identities_.emplace(request.identity.process_id, request.identity.queue_id);
    admission_epoch = admission_epoch_;
    ++active_registrations_;
  }
  RegistrationTransaction transaction(*this, request.identity.process_id,
                                      request.identity.queue_id);
  ...
  QueueHandle handle;
  {
    ...
    if (admission_open_ && admission_epoch_ == admission_epoch &&
        !identity_exists_locked(request.identity)) {
