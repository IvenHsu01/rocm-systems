  ...
  if (soc_ != nullptr) {
    ...
    interrupt_subscription_ =
        amdgpu::InterruptSubscription([this](uint32_t process_id, uint32_t event_id) {
          enqueue_cp_interrupt(process_id, event_id);
        });
  }
...
bool GpuPciDevice::shutdown_frontend() {
  ...
  // Refuse and drain transport work before detaching any queue, translation,
  // interrupt route, or backing storage it may still be using.
  shutdown_transport();
...
void GpuPciDevice::build_block_models() {
  ...
  for (const BlockModelBinding &binding : kBlockModels) {
    ...
    if (block == nullptr) {
      const bool published = std::ranges::any_of(spec_.discovery.blocks, [&](const IpBlock &entry) {
        return entry.hardware_id == binding.id && entry.instance == kModelledInstance;
      });
      // An absent block is optional from the driver's point of view. A required
      // block that is published without any register bases is different: the
      // driver instantiates it but can never address it, so the device cannot
      // safely claim to be usable.
      if (published && binding.required) {
        util::Logger::warn(
            std::format("{}: required block {} instance {} is published without register bases",
                        name(), static_cast<uint16_t>(binding.id), kModelledInstance));
...
void GpuPciDevice::enqueue_cp_interrupt(uint32_t process_id, uint32_t event_id) {
  if (process_id == 0 || process_id > std::numeric_limits<uint16_t>::max()) {
    util::Logger::warn(
        std::format("{}: cannot deliver a CP interrupt for PASID {}", name(), process_id));
    return;
  }
  ...
  }
  {
    ...
    if (!sdma_effect_admission_open_)
      return;
    cp_interrupt_inbox_.push_back({.session = session,
...
void GpuPciDevice::drain_cp_interrupts() {
  std::deque<CpInterruptRequest> requests;
  {
    const std::lock_guard lock(sdma_effect_mutex_);
    requests.swap(cp_interrupt_inbox_);
  }
  for (const CpInterruptRequest &request : requests) {
    const std::shared_ptr<simdojo::PciTransportSession> session = request.session.lock();
    if (session == nullptr || session->generation() != request.generation)
      continue;
    simdojo::PciTransportSession::OperationLease lease = session->acquire();
    FrontendOperationLease frontend = acquire_frontend_operation(request.reset_epoch);
    if (!lease || transport_session() != session || !frontend)
      continue;
    {
      const std::lock_guard lock(sdma_effect_mutex_);
      last_cp_interrupt_thread_ = std::this_thread::get_id();
    }
    if (!deliver_interrupt({.client_id = kGraphicsInterruptClient,
                            .source_id = kCpEndOfPipeSource,
                            .vmid = kFirstComputeVmid,
                            .pasid = static_cast<uint16_t>(request.process_id),
                            .node_id = kFirstXccInterruptNode,
                            .data = {request.event_id}})) {
      util::Logger::warn(std::format("{}: could not deliver CP event {} for PASID {}", name(),
                                     request.event_id, request.process_id));
    }
  }
}
...
void GpuPciDevice::deliver_cp_interrupt_for_test(uint32_t process_id, uint32_t event_id) {
  interrupt_subscription_.sink().deliver(process_id, event_id);
}

std::thread::id GpuPciDevice::last_cp_interrupt_thread_for_test() {
  const std::lock_guard lock(sdma_effect_mutex_);
  return last_cp_interrupt_thread_;
}
...
void GpuPciDevice::reset(simdojo::ResetKind kind) {
  const std::lock_guard reset_lock(doorbell_reset_mutex_);
  close_doorbell_admission_for_reset();
  const bool restart_transport = quiesce_transport_session();
  ...
  try {
    ...
    if (restart_transport && kind != simdojo::ResetKind::LostConnection &&
        !restart_transport_session_after_reset()) {
      throw std::runtime_error("PCI transport session could not be restarted after reset");
    }
  } catch (...) {
    if (restart_transport && kind != simdojo::ResetKind::LostConnection)
      (void)restart_transport_session_after_reset();
