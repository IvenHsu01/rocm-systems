...
constexpr uint32_t kPacket3OneDwordNop = 0xffff1000;
constexpr uint32_t kPacket3SetUconfigRegisterOffsetMask = 0xffff;
...
Pm4PacketProcessResult Pm4PacketProcessor::process(Request request) const {
  ...
  if (result.packet_header == kPacket3OneDwordNop) {
  ...
  const uint32_t encoded_register_offset = request.available_dwords[1];
  if ((encoded_register_offset & ~kPacket3SetUconfigRegisterOffsetMask) != 0) {
    result.packet.status = PacketProcessStatus::Malformed;
    result.diagnostic = Pm4PacketDiagnostic::InvalidRegisterOffset;
    return result;
  }
  result.register_dword = kPacket3SetUconfigRegisterStart +
                          (encoded_register_offset & kPacket3SetUconfigRegisterOffsetMask);
