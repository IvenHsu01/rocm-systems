...
public:
  ...
  void deliver_cp_interrupt_for_test(uint32_t process_id, uint32_t event_id);
  [[nodiscard]] std::thread::id last_cp_interrupt_thread_for_test();
  [[nodiscard]] bool write_register_for_test(uint64_t byte_offset, uint32_t value) {
    return write_register(byte_offset, value);
  }
...
private:
  ...
  [[nodiscard]] bool enqueue_sdma_pci_effect(const std::shared_ptr<SdmaPciCallbackState> &state,
                                             const SdmaPciEffect &effect);
  void enqueue_cp_interrupt(uint32_t process_id, uint32_t event_id);
  void drain_cp_interrupts();
  ...
  /// @brief Cross-thread mailbox keeping compute and SDMA callbacks off PCI state.
  std::mutex sdma_effect_mutex_;
  std::deque<CpInterruptRequest> cp_interrupt_inbox_;
  ...
  std::thread::id last_cp_interrupt_thread_;
