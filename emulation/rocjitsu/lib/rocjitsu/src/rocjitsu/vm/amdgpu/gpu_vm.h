...
public:
  ...
  explicit operator bool() const { return gpu_vm_ != nullptr; }
...
private:
  ...
  GpuVmBindingLease(GpuVm &gpu_vm, AddressSpaceHandle handle, AddressSpaceInfo info)
      : gpu_vm_(&gpu_vm), handle_(handle), info_(info) {}
...
private:
  friend class GpuVmBindingLease;
  ...
  [[nodiscard]] bool release_binding(AddressSpaceHandle handle) noexcept;
