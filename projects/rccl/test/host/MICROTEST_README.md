...
- **`rccl-UnitTestsMicro`** — one unit per test TU:
  ...
  - `ras/ras.cc` (`RAS_CC_PATH`, from `ras-test.cc`); suite
    ...
    loop, timeout handling, and poll-slot management. Network and neighboring
    RAS subsystem calls use test-controlled replacements in `ras-test.cc`,
    their sole consumer. Real multi-process behavior remains integration-test
    territory.
