...
* Fixed a crash in the IB-CAST transport's memory registration and data path: `regMr`, `regMrDmaBuf`, `deregMr`, `isend`, `irecv` and `iflush` dereferenced a null communicator instead of rejecting it, and `regMr` accepted a zero size. These now return `ncclInvalidArgument`. `deregMr` still treats a null memory handle as a successful no-op, ahead of the communicator check, so only a null communicator paired with a non-null handle is rejected.
