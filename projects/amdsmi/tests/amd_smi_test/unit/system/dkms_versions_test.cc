...
TEST(SystemUnit, GetAmdgpuDkmsVersionNullptrAfterInitIsInvalid) {
  ASSERT_EQ(amdsmi_init(0), AMDSMI_STATUS_SUCCESS);

  EXPECT_EQ(amdsmi_get_amdgpu_dkms_version(nullptr, AMDSMI_MAX_STRING_LENGTH), AMDSMI_STATUS_INVAL);
  EXPECT_EQ(amdsmi_shut_down(), AMDSMI_STATUS_SUCCESS);
}

TEST(SystemUnit, GetAmdgpuDkmsVersionShortLengthAfterInitIsInvalid) {
  ASSERT_EQ(amdsmi_init(0), AMDSMI_STATUS_SUCCESS);
  char version[1] = {};

  EXPECT_EQ(amdsmi_get_amdgpu_dkms_version(version, sizeof(version)), AMDSMI_STATUS_INVAL);
  EXPECT_EQ(amdsmi_shut_down(), AMDSMI_STATUS_SUCCESS);
}
