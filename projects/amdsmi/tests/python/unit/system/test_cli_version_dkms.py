...
class TestVersionDkmsOutput(unittest.TestCase):
    ...
    def test_reports_dkms_when_gpu_version_is_disabled(self):
        output = _run_version(lambda: "6.19.20-2450390.24.04", gpu_version=False)
        self.assertEqual(output["amdgpu_dkms_version"], "6.19.20-2450390.24.04")
    ...
    def test_human_readable_includes_dkms_when_gpu_version_is_disabled(self):
        with mock.patch("builtins.print") as printed:
            _run_version(lambda: "6.19.20-2450390.24.04", gpu_version=False, human_readable=True)
        printed.assert_called_once()
        line = printed.call_args[0][0]
        self.assertIn("amdgpu dkms version: 6.19.20-2450390.24.04", line)
        self.assertNotIn("amdgpu version:", line)
